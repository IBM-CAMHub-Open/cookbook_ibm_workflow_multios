...
action :prepare do
  ...
  elsif new_resource.db2_install == 'true'
  ...
  if new_resource.cluster_type != 'SingleCluster' && !new_resource.case_network_shared_dir.nil? && !new_resource.case_network_shared_dir.empty?
    # TODO: avoid using node directly in provider, code refactoring later
    ...
    # for multiple nodes, case_network_shared_dir is MUST and need mount the networkshared directory to local path before create action.
    ...
    converge_by("create #{@new_resource}") do
      ...
      if new_resource.cluster_type == 'SingleClusters'
        ...
        node_hostnames.each do |node_hostname|
          ...
          silent_createde_nodex_propfile_abspath = "#{install_dir}/bin/SingleClusters-Node#{index}-DB2.properties"
          template silent_createde_nodex_propfile_abspath do
            source 'config/SingleClusters-Nodex-DB2.properties.erb'
            variables(
              :NODE_INDEX => index,
...
action :start_dmgr do
  ...
  return unless dmgr_stopped?(workflow_user)
  ...
  execute 'start Deployment Manager' do
    ...
    user workflow_user
    group workflow_group
    only_if { ::Dir.exist?("#{new_resource.install_dir}/profiles/DmgrProfile") }
  end
end
...
action :start_nodeagent do
  ...
  return unless nodeagent_stopped?(workflow_user, nodeIndex)

  execute 'start Node Agent' do
    cwd "#{new_resource.install_dir}/profiles/Node#{nodeIndex}Profile/bin"
...
action :start_server do
  ...
  return unless server_stopped?(workflow_user, nodeIndex, serverName)
...
def database_created?
  ...
  if new_resource.db2_install == 'false'
    true
